...
- 7.4.1 -- ES2015 Set 类的运算
### 7.5 多重集或袋
- 多重集或多重集合是数学中的一个概念, 是集合概念的推广. 在一个集合中, 相同的元素只能
  出现一次, 因此只能显示出有或者无的属性. 在多重集合中, 同一个元素可能出现多次. 正式
  的多重集的概念大约出现在 1970 年代.
- 多重集的势的计算和一般集合的计算方法一样, 出现多次的元素则需要按出现的此处计算, 不能
  只算一次. 一个元素在多重集里出现的次数称为这个元素在多重集里面的重(chong)数
  (或重次,重复度). 举例来说, `{1, 2, 3}` 是一个集合, 而 `{1, 1, 1, 2, 2, 3}` 
  是一个多重集. 其中元素 1 的重数是 3, 2的重数是2, 3的重数是1.  
  `{1, 1, 1, 2, 2, 3}` 的元素个数是 6. 有时为了和一般的集合相区别, 多重集合会用
  方括号而不是花括号来标记, 比如 `{1, 1, 1, 2, 2, 3}` 会被标记为 
  `[1, 1, 1, 2, 2, 3]`. 和 多元组 或 数组 的概念不同, 多重集中的元素是没有顺序分别的,
