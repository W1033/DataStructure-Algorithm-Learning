...
- 03 时间复杂度:
    + 在计算机科学中, 算法的时间复杂度 (Time Complexity) 是一个函数,
      ...
      时间复杂度常用 `大 O 符号` 表述, 不包含这个函数的低阶项和首相系数.
      使用这种方式时, 时间复杂度可被称为是渐近的,
      亦即考察输入值大小趋近无穷时的情况. 例如, 如果一个算法对于任何大小为 `n`
      (必须比 $n_0$)大的输入, 它至多需要 $5n^3 + 3n$ 的时间运行完毕,
      那么它的渐进时间复杂度是 $O(n^3)$.
...
- 选择排序算法是一种`原址比较`排序算法. 选择排序大致的思路是:
  **找到数据结构中的最小值并将其放置在第一位, 接着找到第二小的值并将其放在第二位**,
  ...
  <img src="./chapter13-images/02-selection-sort.png"
...
#### 13.1.5 快速排序 (Quick Sort)
- 快速排序的实现稍微复杂, 详细笔记见同目录的: `./详解JS实现快速排序算法.md`
